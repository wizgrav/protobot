...
class Motor:
  ...
  pins = [ {"pwm":6, "dir":26, "init":True},  {"pwm":19, "dir":13, "init":False},  {"pwm":21, "dir":20, "init":True},  {"pwm":16, "dir":12, "init":True} ]
  ...
  def __init__(self,pi):
    ...
    try:
		...
		pass
  def __del__(self):
    for i in range(0,4): self.pi.set_PWM_dutycycle(self.pins[i]["pwm"] ,0)
    try:
		self.pi.clear_bank_1(0xffffffff)
    except:
		pass
  ...
  def con(self,v,i):
    if v != 0:
        v = (1 if v>0 else -1)*int(self.vmin+abs(v)*(self.vmax-self.vmin))
  ...
  def set(*m):
    ...
    if a:
      ...
      for i in range(0,4):
        ...
        cl = cl | r[0]
        st = st | r[1]
      if cl: m[0].pi.clear_bank_1(cl)
      if st: m[0].pi.set_bank_1(st)
